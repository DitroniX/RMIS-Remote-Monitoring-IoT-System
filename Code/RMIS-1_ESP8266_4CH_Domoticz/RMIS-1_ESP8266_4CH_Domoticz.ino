...
void loop() {
  ...
  if (Index_Temperature > 0) {
    // Read PCB_Temp Sensor Temperature ÂºC and round to 1 decimal place
    SensorRAW = TempSensor.readTempC();
    dtostrf (SensorRAW, 6, 1, SensorResult);
    sprintf (SensorResult, "%s", SensorResult);
    Serial.println(String (SensorResult) + "\t Temperature Â°C (" + String(int(SensorRAW)) + ")\t Last_Temperature" + " #" + String(Index_Temperature));
